...
type PageKey = "overview" | "customers" | "entries" | "payments" | "settings";
...
const NAV_ITEMS: NavItem[] = [
  { href: "/", label: "Overview", page: "overview", icon: "📊" },
  { href: "/customers", label: "Customers", page: "customers", icon: "👥" },
  { href: "/entries", label: "Time Entries", page: "entries", icon: "⏱️" },
  { href: "/payments", label: "Payments", page: "payments", icon: "💶" },
  { href: "/settings", label: "Data", page: "settings", icon: "🗂️" },
];
...
export default function DashboardApp() {
  ...
  const [sparkBurstVisible, setSparkBurstVisible] = useState(false);
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => setRewardVisible(false), 1800);
    return () => window.clearTimeout(id);
  }, [rewardVisible]);

  useEffect(() => {
    if (!sparkBurstVisible) {
      return;
    }
    const id = window.setTimeout(() => setSparkBurstVisible(false), 950);
    return () => window.clearTimeout(id);
  }, [sparkBurstVisible]);
  ...
  const totalTrackedHours = useMemo(
    () => data.entries.reduce((sum, entry) => sum + getDurationHours(entry), 0),
    [data.entries],
  );

  const totalBilled = useMemo(
    () => data.entries.reduce((sum, entry) => sum + getDurationHours(entry) * entry.hourlyRate, 0),
    [data.entries],
  );
  ...
  const addPayment = () => {
    ...
    const dateIso = toIsoDate(paymentDate);
    if (!dateIso) {
      return;
    }
  ...
  const recentEntries = [...data.entries].sort((a, b) => b.startIso.localeCompare(a.startIso)).slice(0, 4);
  const topCustomer = [...customerOverview].sort((a, b) => b.totalBilled - a.totalBilled)[0];

  if (!loaded) {
    return <main className={styles.loadingPage}>Loading dashboard…</main>;
  }
  ...
  return (
    <main className={styles.dashboardPage}>
      <aside className={styles.sidebar}>
        <div>
          <p className={styles.brandKicker}>Time Intelligence</p>
          <h1 className={styles.brand}>Lovely Dashboard</h1>
        </div>
        <nav className={styles.nav} aria-label="Main navigation">
          {NAV_ITEMS.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`${styles.navLink} ${activePage === item.page ? styles.navLinkActive : ""}`}
            >
      ...
      <section className={styles.mainContent}>
        <header className={styles.hero}>
          <div>
            <p className={styles.heroKicker}>Performance dashboard</p>
            <h2>Track work, revenue, and momentum in real time</h2>
            <p className={styles.heroText}>
              Your time tracking data now lives in a multi-page dashboard layout with quick insights.
            </p>
          </div>
          <div className={styles.heroBalance}>
            <span>Open balance</span>
            <strong>{formatCurrency.format(totalOwed)}</strong>
          </div>
          {rewardVisible && <span className={styles.reward}>✨ Session captured — momentum +1</span>}
          {sparkBurstVisible && (
            <span className={styles.sparkBurst} aria-hidden>
              ✦ ✧ ✦
            </span>
          )}
        </header>
        ...
        <section className={styles.statsGrid}>
          <article className={styles.statCard}>
            <p>Tracked hours</p>
            <strong>{totalTrackedHours.toFixed(1)}h</strong>
          </article>
          <article className={styles.statCard}>
            <p>Total billed</p>
            <strong>{formatCurrency.format(totalBilled)}</strong>
          </article>
          <article className={styles.statCard}>
            <p>Total paid</p>
            <strong>{formatCurrency.format(totalPaid)}</strong>
          </article>
          <article className={styles.statCard}>
            <p>Top customer</p>
            <strong>{topCustomer ? topCustomer.customer.name : "—"}</strong>
          </article>
        </section>
              ...
              <div className={styles.list}>
                {recentEntries.map((entry) => (
                  <article key={entry.id} className={styles.listItem}>
                    <p>
                      <strong>{customerNames.get(entry.customerId) ?? "Unknown"}</strong> · {getDurationHours(entry).toFixed(2)}h
                    </p>
                    <p className={styles.meta}>{entry.description || "No description"}</p>
                  </article>
                ))}
                {recentEntries.length === 0 && <p className={styles.meta}>No tracked sessions yet.</p>}
              </div>
            </section>
          </>
        )}
        ...
        {activePage === "customers" && (
          <section className={styles.card}>
            ...
            <div className={styles.list}>
              {customerOverview.map(({ customer, trackedHours, totalBilled: billed, totalPaid: paid, owed }) => (
                <article key={customer.id} className={styles.listItem}>
                  ...
                  <p className={styles.meta}>
                    {trackedHours.toFixed(2)}h tracked · Billed {formatCurrency.format(billed)} · Paid {" "}
                    {formatCurrency.format(paid)} · Owed <strong>{formatCurrency.format(owed)}</strong>
                  </p>
              ...
              {customerOverview.length === 0 && <p className={styles.meta}>No customers yet.</p>}
            </div>
          </section>
        )}
                    ...
                    <div className={styles.grid}>
                      ...
                      <label>
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            const dateIso = toIsoDate(event.target.value);
                            if (dateIso) {
                              updatePayment(payment.id, { dateIso });
                            }
                          }}
                        />
                      </label>
        ...
        {activePage === "settings" && (
          <section className={styles.card}>
            <h3>Import / export</h3>
            <p className={styles.meta}>Download your data for backup, or restore from an exported JSON file.</p>
            <div className={styles.row}>
